...
module.exports = function(grunt) {
    ...
    */
    ...
    Page.prototype = {
        ...
        */
        getData: function () {
            ...
            if (GLOBAL.options.env) {
                data = getJSON(dataFile.replace('.json', '.' + GLOBAL.options.env + '.json'), this.$data.copy);
            }
            if (!data) {
                data = getJSON(dataFile, this.$data.copy);
            }
        ...
        */
        create: function () {
            var html = mustache.render(grunt.file.read(this.$page), this.$data.content, GLOBAL.partials.$data);
    ...
    grunt.registerMultiTask('mustacheGenerate', 'Grunt task to generate html pages and optionally partials for reuse client side.', function () {
        ...
        */
        ...
        if (GLOBAL.options.globalData) {
            GLOBAL.data = getJSON(GLOBAL.options.globalData);
        }
